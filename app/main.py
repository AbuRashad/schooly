...
_CAMERA_AGENT_SCAN_INTERVAL_SECONDS = 10
_CV_AGENT_SCAN_INTERVAL_SECONDS = 6
...
async def _camera_agent_monitoring_loop() -> None:
    """Background task: evaluate camera health and update alert states."""
    while True:
        await asyncio.sleep(_CAMERA_AGENT_SCAN_INTERVAL_SECONDS)
        try:
            active = camera_alert_agent.scan_once()
            if active:
                logger.info("Camera agent active alert(s): %d", len(active))
        except Exception:  # noqa: BLE001
            logger.exception("Camera monitoring agent scan failed")


async def _computer_vision_agent_loop() -> None:
    """Background task: analyze video streams and update CV risk alerts."""
    while True:
        await asyncio.sleep(_CV_AGENT_SCAN_INTERVAL_SECONDS)
        try:
            active = computer_vision_agent.scan_once()
            if active:
                logger.info("Computer vision agent active risk(s): %d", len(active))
        except Exception:  # noqa: BLE001
            logger.exception("Computer vision agent scan failed")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Start camera monitoring agent loop for real-time alerting.
    camera_agent_task = asyncio.create_task(_camera_agent_monitoring_loop())
    logger.info(
        "Camera monitoring agent scheduled (interval: %ss)",
        _CAMERA_AGENT_SCAN_INTERVAL_SECONDS,
    )

    cv_agent_task = asyncio.create_task(_computer_vision_agent_loop())
    logger.info(
        "Computer vision agent scheduled (interval: %ss)",
        _CV_AGENT_SCAN_INTERVAL_SECONDS,
    )
    ...
    try:
        ...
        harvest_task.cancel()
        camera_agent_task.cancel()
        cv_agent_task.cancel()
