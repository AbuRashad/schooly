...
import {
  Users, CalendarCheck, BarChart2, LogOut, Menu, X, ChevronRight,
  FileBarChart, ScanFace, UserCheck, UserX, Camera, CheckCircle,
...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function handleSignOut() { await authClient.signOut(); navigate('/dashboard/login'); }
  return (
    <>
      {open && <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={onClose} />}
      <aside
        className={`fixed top-0 right-0 h-full w-60 z-50 flex flex-col transition-transform duration-300 md:translate-x-0 md:static md:z-auto ${open ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}
      ...
      >
        <div className="flex items-center gap-3 px-5 py-5 border-b border-white/5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
            style={{ background: 'linear-gradient(135deg,#00C2FF22,#22C55E22)', border: '1.5px solid #00C2FF55' }}>👁️</div>
          <div>
