...
import {
  ...
  useStudentPortal,
  useAgentInsights,
  useCameraAgentAlerts,
  useComputerVisionAgentAlerts,
...
export default function MonitoringPage() {
  ...
  const cameraAgentAlerts = useCameraAgentAlerts();
  const computerVisionAlerts = useComputerVisionAgentAlerts();

  const mergedAgentAlerts = [...computerVisionAlerts, ...cameraAgentAlerts].slice(0, 20);
  const liveAlerts = mergedAgentAlerts.length > 0 ? mergedAgentAlerts : snapshot.liveAlerts;
      ...
      <main className="grid gap-4 content-start">
        ...
        {page === "dashboard" && (
          <>
            ...
            <AIInsightsPanel ssi={snapshot.ssi} benchmark={snapshot.benchmark} alerts={liveAlerts} agentInsights={agentInsights} />
          </>
        )}
