...
// ═══════════════════════════════════════════════════════════════════════
// NEW TABLES FOR 11-FEATURE REVOLUTION
// ═══════════════════════════════════════════════════════════════════════
...
export const badgeDefinitions = mysqlTable('badge_definitions', {
  ...
  tier: mysqlEnum('badge_tier', ['bronze', 'silver', 'gold', 'platinum']).notNull().default('bronze'),
  criteriaType: varchar('criteria_type', { length: 50 }).notNull(), // attendance_streak, grade_excellent, participation, etc.
  criteriaValue: int('criteria_value').notNull().default(1),
...
export const studentBadges = mysqlTable('student_badges', {
  ...
  awardedBy: int('awarded_by').references(() => staffProfiles.id, { onDelete: 'set null' }),
  context: text('context'), // e.g. "15 days attendance streak"
}, (table) => ({
  studentBadgeUnique: uniqueIndex('uq_student_badge').on(table.studentId, table.badgeId),
  studentIdx: index('idx_student_badges_student').on(table.studentId),
}));
...
export const studentPoints = mysqlTable('student_points', {
  ...
  category: mysqlEnum('point_category', ['attendance', 'grade', 'behavior', 'participation', 'achievement', 'other']).notNull().default('other'),
  awardedBy: int('awarded_by').references(() => staffProfiles.id, { onDelete: 'set null' }),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  pointsStudentIdx: index('idx_points_student').on(table.studentId),
  pointsCategoryIdx: index('idx_points_category').on(table.category),
}));

// ── Feature 2: AI Risk Radar ─────────────────────────────────────────

export const riskAlerts = mysqlTable('risk_alerts', {
  id: int('id').primaryKey().autoincrement(),
  studentId: int('student_id').notNull().references(() => students.id, { onDelete: 'cascade' }),
  riskScore: int('risk_score').notNull(), // 0-100
  factors: text('factors'), // JSON: {attendance: 0.3, grades: 0.4, ...}
  severity: mysqlEnum('risk_severity', ['low', 'medium', 'high', 'critical']).notNull().default('low'),
  status: mysqlEnum('risk_status', ['open', 'acknowledged', 'resolved', 'escalated']).notNull().default('open'),
  suggestedAction: text('suggested_action'),
  acknowledgedBy: int('acknowledged_by').references(() => staffProfiles.id, { onDelete: 'set null' }),
  acknowledgedAt: timestamp('acknowledged_at'),
  resolvedAt: timestamp('resolved_at'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  riskStudentIdx: index('idx_risk_student').on(table.studentId),
  riskSeverityIdx: index('idx_risk_severity').on(table.severity, table.status),
  riskCreatedIdx: index('idx_risk_created').on(table.createdAt),
}));

// ── Feature 3: Parent Pulse ──────────────────────────────────────────

export const parentNotifications = mysqlTable('parent_notifications', {
  id: int('id').primaryKey().autoincrement(),
  guardianId: int('guardian_id').notNull().references(() => guardianProfiles.id, { onDelete: 'cascade' }),
  studentId: int('student_id').notNull().references(() => studentProfiles.id, { onDelete: 'cascade' }),
  type: mysqlEnum('parent_notif_type', ['attendance', 'grade', 'behavior', 'announcement', 'alert', 'achievement']).notNull(),
  title: varchar('title', { length: 200 }).notNull(),
  body: text('body').notNull(),
  ...
  notifGuardianIdx: index('idx_notif_guardian').on(table.guardianId, table.isRead),
  notifStudentIdx: index('idx_notif_student').on(table.studentId),
  notifCreatedIdx: index('idx_notif_created').on(table.createdAt),
}));

// ── Feature 6: Mood Compass ──────────────────────────────────────────

export const moodLogs = mysqlTable('mood_logs', {
  id: int('id').primaryKey().autoincrement(),
  studentId: int('student_id').notNull().references(() => studentProfiles.id, { onDelete: 'cascade' }),
  moodScore: int('mood_score').notNull(), // 1-5
  note: text('note'),
  date: date('date').notNull(),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  moodStudentDateUnique: uniqueIndex('uq_mood_student_date').on(table.studentId, table.date),
  moodStudentIdx: index('idx_mood_student').on(table.studentId),
  moodDateIdx: index('idx_mood_date').on(table.date),
}));

// ── Feature 8: PeerReview Hub ────────────────────────────────────────

export const peerReviews = mysqlTable('peer_reviews', {
  id: int('id').primaryKey().autoincrement(),
  assignmentSubmissionId: int('submission_id').notNull().references(() => assignmentSubmissions.id, { onDelete: 'cascade' }),
  reviewerId: int('reviewer_id').notNull().references(() => studentProfiles.id, { onDelete: 'cascade' }),
  revieweeId: int('reviewee_id').notNull().references(() => studentProfiles.id, { onDelete: 'cascade' }),
  clarityScore: int('clarity_score'), // 1-5
  accuracyScore: int('accuracy_score'), // 1-5
  completenessScore: int('completeness_score'), // 1-5
  creativityScore: int('creativity_score'), // 1-5
  overallComment: text('overall_comment'),
  aiSuggestedImprovements: text('ai_suggested_improvements'),
  isSubmitted: boolean('is_submitted').notNull().default(false),
  submittedAt: timestamp('submitted_at'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  peerReviewSubmissionUnique: uniqueIndex('uq_peer_review_submission_reviewer').on(table.assignmentSubmissionId, table.reviewerId),
  peerReviewReviewerIdx: index('idx_peer_review_reviewer').on(table.reviewerId),
}));

// ── Feature 9: Smart Inventory ───────────────────────────────────────
...
export const inventoryItems = mysqlTable('inventory_items', {
  ...
  minThreshold: int('min_threshold').notNull().default(10),
  ...
  inventorySchoolCodeUnique: uniqueIndex('uq_inventory_school_code').on(table.schoolId, table.code),
  inventoryCategoryIdx: index('idx_inventory_category').on(table.schoolId, table.category),
}));

export const inventoryTransactions = mysqlTable('inventory_transactions', {
  id: int('id').primaryKey().autoincrement(),
  itemId: int('item_id').notNull().references(() => inventoryItems.id, { onDelete: 'cascade' }),
  type: mysqlEnum('inventory_tx_type', ['in', 'out', 'adjustment']).notNull(),
  quantity: int('quantity').notNull(),
  reason: text('reason'),
  performedBy: int('performed_by').references(() => staffProfiles.id, { onDelete: 'set null' }),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  txItemIdx: index('idx_inventory_tx_item').on(table.itemId),
}));

// ── Feature 10: Blockchain-Verified Certificates ─────────────────────

export const certificates = mysqlTable('certificates', {
  id: int('id').primaryKey().autoincrement(),
  studentId: int('student_id').notNull().references(() => studentProfiles.id, { onDelete: 'cascade' }),
  schoolId: int('school_id').notNull().references(() => schools.id, { onDelete: 'cascade' }),
  type: mysqlEnum('certificate_type', ['transcript', 'graduation', 'achievement', 'attendance']).notNull(),
  ...
  certHashUnique: uniqueIndex('uq_cert_hash').on(table.hash),
  certStudentIdx: index('idx_cert_student').on(table.studentId),
}));
